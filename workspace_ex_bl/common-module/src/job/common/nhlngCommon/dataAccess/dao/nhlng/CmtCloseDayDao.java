...
import job.common.nhlngCommon.dataAccess.mapper.nhlng.CmtCloseDayMapper;
...
public class CmtCloseDayDao{
	...
	private static void convertMapListToModelList(final List<Map<COLUMNS, Object>> mapList, final List<CmtCloseDay> objectList) throws InstantiationException, IllegalAccessException {
		if (mapList == null) { return; }
		for (Map<COLUMNS, Object> map : mapList) {
			CmtCloseDay object = new CmtCloseDay();
	...
	private static void convertMapToModel(final Map<COLUMNS, Object> map, final CmtCloseDay object) {
		if (map == null) { return; }
	...
	private static void convertExMapToModel(final Map<EX_COLUMNS, Object> map, final CmtCloseDay object) {
		if (map == null) {
			return;
		}
		object.setCloseDtime((Date) map.get(COLUMNS.CLOSE_DTIME));
	...
	private static void convertModelListToMapList(final List<CmtCloseDay> objectList, final List<Map<COLUMNS, Object>> mapList) {
		if (objectList == null) { return; }
		for (CmtCloseDay object : objectList) {
	...
	private static void convertModelToMap(final CmtCloseDay object, final Map<COLUMNS, Object> data) {
	...
	private static void convertModelListToExMapList(final List<CmtCloseDay> objectList,
	...
	private static void convertModelToExMap(final CmtCloseDay object, final Map<EX_COLUMNS, Object> data) {
		if (object == null) {
			return;
		}
		if (object.getCloseDtime() != null) {
	...
	private static CmtCloseDayExample makeEqualToMatchingExample(final Map<COLUMNS, Object> searchingOption) {
		// WHERE clause
	...
	private static CmtCloseDayExample makePartialMatchingExample(final Map<COLUMNS, Object> searchingOption) {
		// WHERE clause
		CmtCloseDayExample example = new CmtCloseDayExample();
		if (searchingOption.size() > 0) {
			Date value1 = (Date) searchingOption.get(COLUMNS.CLOSE_DTIME);
	...
	private static CmtCloseDayExample makeFromToMatchingExample(final Map<COLUMNS, Object> searchingOption, final Date toDate) {
		// WHERE clause
		CmtCloseDayExample example = new CmtCloseDayExample();
		if (searchingOption.size() > 0) {
			Date value1 = (Date) searchingOption.get(COLUMNS.CLOSE_DTIME);
	...
	public static long countEqualToMatching(final SqlSession session, final Map<COLUMNS, Object> searchingOption) {
		CmtCloseDayExample example = makeEqualToMatchingExample(searchingOption);
		return count(session, example);
	}
	 ...
	 * @return
	 */
	public static long countPartialMatchingMatching(final SqlSession session, final Map<COLUMNS, Object> searchingOption) {
		CmtCloseDayExample example = makePartialMatchingExample(searchingOption);
		return count(session, example);
	}
	 ...
	 * @return
	 */
	public static long countFromToMatching(final SqlSession session, final Map<COLUMNS, Object> searchingOption, Date toDate) {
		CmtCloseDayExample example = makeFromToMatchingExample(searchingOption, toDate);
		return count(session, example);
	}
	...
	private static long count(final SqlSession session, final CmtCloseDayExample example) {
	 ...
	 * @return
	 */
	public static Map<COLUMNS, Object> selectByPrimaryKey(final SqlSession session, final Date closeDtime,final String tagNo) {
		CmtCloseDayMapper mapper = session.getMapper(CmtCloseDayMapper.class);
		CmtCloseDay record = mapper.selectByPrimaryKey(closeDtime, tagNo);
	 ...
	 * @return
	 */
	public static List<Map<COLUMNS, Object>> makeFromToMatchingExample(final SqlSession session, final Map<COLUMNS, Object> searchingOption, Date toDate) {
		CmtCloseDayExample example = makeFromToMatchingExample(searchingOption, toDate);
		return select(session, example);
	}
	...
	private static List<Map<COLUMNS, Object>> select(final SqlSession session, final CmtCloseDayExample example) {
		// ORDER BY clause
		example.setOrderByClause(CLOSEDTIME_COLUMN_NAME);

		// Select.
		CmtCloseDayMapper mapper = session.getMapper(CmtCloseDayMapper.class);
		List<CmtCloseDay> recordList = mapper.selectByExample(example);
	...
	private static List<Map<CmtCloseDayDao.EX_COLUMNS, Object>> ExSelect(final SqlSession session,
		...
		// Select.
		CmtCloseDayMapper mapper = session.getMapper(CmtCloseDayMapper.class);
		List<CmtCloseDay> recordList = mapper.selectByExtendedExample(example);
	...
	public static int insert(final SqlSession session, final Map<COLUMNS, Object> data) {
		...
		// Make data.
		CmtCloseDay record = new CmtCloseDay();
	...
	public static int insertOrUpdate(final SqlSession session, final List<Map<COLUMNS, Object>> dataList) {
		...
		List<CmtCloseDay> recordList = new ArrayList();
		try {
